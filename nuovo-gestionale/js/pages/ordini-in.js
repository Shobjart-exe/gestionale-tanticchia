// ordini-in.js - Lista ordini IN (attesa/evasi) da localStorage
...
const OrdiniInPage = {
  ...
  init() {
    ...
    // Se la pagina non passa parametri, deduci dallo filename
    // e forza anche la query (utile per coerenza)
    if (!params.stato) {
  ...
  load() {
    this.ordini = Storage.getOrdiniIn();
  ...
  bind() {
    ...
    if (nuovoBtn) {
      nuovoBtn.onclick = () => {
        window.location.href = `dettaglio.html?mode=create&tipo=in`;
      };
    }
  },
...
document.addEventListener('DOMContentLoaded', () => OrdiniInPage.init());
